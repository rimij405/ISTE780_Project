...
def task_create_env():
    ...
    def create_env():
        ...
        if not env_active():
            return TaskFailed(
                "Cannot complete task without using correct conda environment."
            )
        if env_exists():
...
def task_update_env():
    """
    Update existing conda environment using 'environment.yaml'.
    """
    ...
    def update_env():
        """
        Update local prefixed conda environment using the command line.
        """
        if env_exists():
            return "conda env update --prefix=.condaenv --file=environment.yaml"
        return TaskFailed("Cannot update non-existing environment.")
